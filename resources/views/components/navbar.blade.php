            ...
            <div class="hidden md:flex items-center gap-6">
                    ...
                    <div class="relative" @click.away="isProfileOpen = false">
                        @if (auth()->check())
                            ...
                            <div x-show="isProfileOpen" x-transition
                                class="absolute right-0 mt-2 w-48 bg-gray-900 rounded-lg shadow-lg border border-white/10">
                                <a href="/profile"
                                    class="block px-4 py-2 text-sm text-white hover:bg-gray-700">Profil</a>
                                <a href="/myarticle"
                                    class="block px-4 py-2 text-sm text-white hover:bg-gray-700">Artikel Saya</a>
                                <a href="/myblogs" class="block px-4 py-2 text-sm text-white hover:bg-gray-700">Blog
                                    Saya</a>

                                <a href="/settings"
                                    class="block px-4 py-2 text-sm text-white hover:bg-gray-700">Pengaturan</a>
                        ...
                        @else
                            <!-- Login Button -->
                            <a href="{{ route('login') }}"
                                class="bg-purple-500 text-white px-4 py-2 rounded-lg hover:bg-purple-600 transition">Login</a>
                        @endif
                    </div>
                </div>
                ...
                <div class="md:hidden">
                    <button @click="isOpen = !isOpen" class="p-2">
                        <div>
                            <span class="block w-6 h-0.5 bg-white transform transition duration-300"
                                :class="{ 'rotate-45 translate-y-1.5': isOpen, 'translate-y-0': !isOpen }"></span>
                            <span class="block w-6 h-0.5 bg-white my-1.5 transition duration-300"
